...
namespace Match3Game
{
    ...
    public class Board : MonoBehaviour
    {
        ...
        private IEnumerator 啟動交換寶石(int x1, int y1, int x2, int y2)
        {
            ...
            while (gem1.isAnimating || gem2.isAnimating)
            {
                yield return null;
            }
            ...
            {
                // 觸發組合效果
                int combinedEffectType = 處理特殊寶石的組合(gem1, gem2);
                Debug.Log($"兩個特殊寶石交換，組合效果類型: {combinedEffectType}");

                if (combinedEffectType >= 0)
                {
                    // 觸發組合效果
                    yield return StartCoroutine(specialGemActivator.觸發特殊寶石(gem1, combinedEffectType));
                }
                else
                {
                    // 分別觸發兩個特殊寶石效果
                    yield return StartCoroutine(specialGemActivator.觸發特殊寶石(gem1, gem1.id - 100));
                    yield return new WaitForSeconds(DESTROY_DELAY);
                    yield return StartCoroutine(specialGemActivator.觸發特殊寶石(gem2, gem2.id - 100));
                }

                // 清理原始寶石
                gems[x1, y1] = null;
                gems[x2, y2] = null;
                if (gem1.gameObject != null) Destroy(gem1.gameObject);
                if (gem2.gameObject != null) Destroy(gem2.gameObject);
            }
            else if (gem1.id >= 100 || gem2.id >= 100)
            {
                // 單個特殊寶石的處理
                var specialGem = gem1.id >= 100 ? gem1 : gem2;
                yield return StartCoroutine(specialGemActivator.觸發特殊寶石(specialGem, specialGem.id - 100));
            }
            else
            {
        ...
        private IEnumerator 延遲處理特殊寶石(Gem swappedGem1, Gem swappedGem2)
        {
            ...
            if (處理特殊寶石的組合(swappedGem1, swappedGem2) == -1)
            {
                寶石交換已完成(true);
            }
        }
        ...
        private int 處理特殊寶石的組合(Gem first, Gem second)
        {
            if (first == null || second == null) return -1;

            // 獲取特殊寶石ID（減去100得到實際類型）
            int type1 = first.id - 100;
            int type2 = second.id - 100;
            ...
            {
                // 相同類型的組合
                (0, 0) => 104,  // LineH + LineH = Cross
                (1, 1) => 104,  // LineV + LineV = Cross
                (2, 2) => 105,  // Bomb + Bomb = BigBomb
                (3, 3) => 106,  // Rainbow + Rainbow = DestroyAll

                // 不同類型的組合
                (0, 1) => 104,  // LineH + LineV = Cross
                (0, 2) => 107,  // LineH + Bomb = RandomHLines
                (0, 3) => 110, // LineH + Rainbow = ThreeHLines
                (1, 2) => 108,  // LineV + Bomb = RandomVLines
                (1, 3) => 111, // LineV + Rainbow = ThreeVLines
                (2, 3) => 109,  // Bomb + Rainbow = MultiBomb

                // 其他組合返回-1，表示不產生新的特殊寶石
                _ => -1
            };
        }
        ...
        private IEnumerator 處理配對序列()
        {
            ...
            {
                // 1. 先收集所有配對資訊
                List<Gem> allMatchedGems = matches.SelectMany(m => m.matchedGems).Distinct().ToList();

                // 2. 先檢查是否要產生特殊寶石，儲存資訊但還不產生
                List<(int x, int y, int resourceType)> specialGemsToCreate = new List<(int x, int y, int resourceType)>();

                foreach (var match in matches.Where(m => m.matchedGems.Count >= 4))
                {
                    var (resourceType, isHorizontal, isVertical, _) = DetectMatchAndDetermineResourceType(match.matchedGems);
                    ...
                    if (resourceType >= 0)
                    {
                        ...
                        {
                            specialGemsToCreate.Add((triggerX, triggerY, resourceType));
                        }
                        // 找替代位置
                        else if (TryFindAlternativePosition(out int newX, out int newY))
                        {
                            specialGemsToCreate.Add((newX, newY, resourceType));
                        }
                    }
                }
                ...
                {
                    if (gem != null && gems[gem.x, gem.y] == gem)
                    {
                ...
                // 執行消失動畫
                yield return StartCoroutine(淡出與刪除寶石(new HashSet<Gem>(allMatchedGems)));
                yield return new WaitForSeconds(DESTROY_DELAY);

                // 4. 產生特殊寶石
                foreach (var (x, y, resourceType) in specialGemsToCreate)
                {
                    生成特殊寶石(x, y, resourceType);
                    yield return new WaitForSeconds(DESTROY_DELAY);
                }
        ...
        // 使寶石下落並填補空白位置的協程
        // 這是遊戲板動態重組的核心邏輯
        public IEnumerator 落下寶石()
        {
            bool hasFalling;
            HashSet<Gem> animatingGems = new HashSet<Gem>();
            ...
            do
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    for (int y = 0; y < height - 1; y++)
                    {
                        if (gems[x, y] == null && gems[x, y + 1] != null)
                        {
                            ...
                            // 開始下落動畫
                            StartCoroutine(gem.AnimateMove(new Vector3(x, y, 0), FALL_DELAY));
                            animatingGems.Add(gem);
                            hasFalling = true;
                        }
                    }
                }
                ...
                if (hasFalling)
                {
                    ...
                    {
                        isAnimating = false;
                        foreach (var gem in animatingGems)
                        {
                            if (gem != null && gem.isAnimating)
                            {
                                isAnimating = true;
                                break;
                            }
        ...
        {
            Debug.Log("消失與刪除寶石: " + gemsToDestroy.Count);
